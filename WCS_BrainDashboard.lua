...
function Dashboard:CollectMetrics()
    ...
    if UpdateAddOnMemoryUsage then
        ...
        local memory = GetAddOnMemoryUsage("WCS_Brain")
        self.metrics.memoryUsage = memory or 0
    else
        self.metrics.memoryUsage = 0
    end
    
    -- CPU estimado (basado en memoria y eventos)
    local cpuEstimate = 0
    if self.metrics.eventsProcessed > 0 then
        cpuEstimate = (self.metrics.memoryUsage / 1024) * 0.1 -- Estimación simple
    end
    self.metrics.cpuUsage = cpuEstimate
    
    -- Eventos (desde EventThrottle si existe)
    if WCS_BrainEventThrottle and WCS_BrainEventThrottle.State then
        self.metrics.eventsProcessed = WCS_BrainEventThrottle.State.totalProcessed or 0
        self.metrics.eventsThrottled = WCS_BrainEventThrottle.State.totalBlocked or 0
    end
    ...
    -- Pet Cooldowns
    local petCdCount = 0
    if WCS_Brain.PetAI and WCS_Brain.PetAI.cooldowns then
    ...
    -- Decisiones de IA (contar desde historial si existe)
    if WCS_Brain and WCS_Brain.BrainAI and WCS_Brain.BrainAI.History then
        self.metrics.aiDecisions = table.getn(WCS_Brain.BrainAI.History)
    end
    
    if WCS_Brain and WCS_Brain.PetAI and WCS_Brain.PetAI.History then
        self.metrics.petAIDecisions = table.getn(WCS_Brain.PetAI.History)
    end
