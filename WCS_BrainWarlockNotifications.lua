...
function WarlockNotif:Initialize()
    -- Crear frame para eventos de inicialización
...
function WarlockNotif:DelayedInitialize()
    ...
    if not WCS_BrainNotifications then
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WCS Warlock]|r WCS_BrainNotifications no encontrado")
        self.enabled = false
        return
    end
    
    -- Verificar que sea Warlock
    local _, class = UnitClass("player")
    if not class then
        -- Clase aún no disponible, reintentar
        return
    end
    
    if class ~= "WARLOCK" then
        -- No es warlock, deshabilitar silenciosamente
        self.enabled = false
        return
    end
    ...
    -- Desregistrar el frame de inicialización
    ...
    self.frame:RegisterEvent("PLAYER_REGEN_DISABLED") -- Entrar en combate
    self.frame:RegisterEvent("PLAYER_REGEN_ENABLED")  -- Salir de combate
    
    self.frame:SetScript("OnEvent", function()
        WarlockNotif:OnEvent(event, arg1)
    end)
    
    -- Frame para actualizaciones periódicas
    self.updateFrame = CreateFrame("Frame")
    self.updateFrame:SetScript("OnUpdate", function()
        WarlockNotif:OnUpdate(arg1)
    end)
    
    DEFAULT_CHAT_FRAME:AddMessage("|cFF9482C9WCS Warlock Notifications|r v" .. self.VERSION .. " cargado")
end
